...
test_that(desc="mark_my_assignment()",{
  ...
  x <- capture.output(y <- mark_my_assignment(tasks = "task1", quiet = TRUE))
  expect_is(y, "testthat_results")
  ...
  x <- capture.output(y <- mark_my_assignment(force_get_tests = TRUE, quiet = TRUE))
  expect_is(y, "testthat_results")
  
  x <- capture.output(expect_warning(mark_my_assignment(mark_file = file.path(system.file(package = "markmyassignment"), "extdata/example_lab_file.R"), quiet = TRUE)))
